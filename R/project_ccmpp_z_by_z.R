...
### Two possible migration assumptions: end of period or evenly over period
  ...
  # compute deaths from year 0 population and survival ratios
    death_count_age_m <- (1-survival_ratio_age_m)*lag_pop_count_age_m_begin_mig_adj
    death_count_age_m[nage] <- (1-survival_ratio_age_m[nage]) * (pop_count_age_m_begin_mig_adj[nage-1]+pop_count_age_m_begin_mig_adj[nage])
    
    death_count_age_f <- (1-survival_ratio_age_f)*lag_pop_count_age_f_begin_mig_adj
    death_count_age_f[nage] <- (1-survival_ratio_age_f[nage]) * (pop_count_age_f_begin_mig_adj[nage-1]+pop_count_age_f_begin_mig_adj[nage])

  # project population by age at year +z from year 0 population and deaths
    pop_count_age_m_end <- lag_pop_count_age_m_begin_mig_adj - death_count_age_m + mig_net_count_age_m_end
    pop_count_age_m_end[nage] <- pop_count_age_m_begin_mig_adj[nage-1] + pop_count_age_m_begin_mig_adj[nage] - death_count_age_m[nage] + mig_net_count_age_m_end[nage]
    
    pop_count_age_f_end <- lag_pop_count_age_f_begin_mig_adj - death_count_age_f + mig_net_count_age_f_end
    pop_count_age_f_end[nage] <- pop_count_age_f_begin_mig_adj[nage-1] + pop_count_age_f_begin_mig_adj[nage] - death_count_age_f[nage] + mig_net_count_age_f_end[nage]
  ...
  # compute infant deaths from total births by sex and survival ratio
    death_count_age_m[1] <- (1 - survival_ratio_age_m[1]) * birth_count_tot_m
    death_count_age_f[1] <- (1 - survival_ratio_age_f[1]) * birth_count_tot_f

  # project infant population at year +1 from births and infant deaths
    pop_count_age_m_end[1] <- birth_count_tot_m - death_count_age_m[1] + mig_net_count_age_m_end[1]
    pop_count_age_f_end[1] <- birth_count_tot_f - death_count_age_f[1] + mig_net_count_age_f_end[1]
  ...
  # assemble list of outputs and return
    ...
    proj_list <- list(age=c(seq(0,(nage-1)*z, z)),
                      ...
                      death_count_age_m = death_count_age_m,
                      death_count_age_f = death_count_age_f,
