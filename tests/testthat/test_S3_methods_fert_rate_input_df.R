context("Tst methods for S3 class 'fert_rate_input_df'")
...
test_that("subsetting drops class", {
    ...
    expect_not_s3_class(y[, "age_start"], "fert_rate_input_df")
    expect_not_s3_class(y$age_start, "fert_rate_input_df")
    expect_not_s3_class(y[["age_start"]], "fert_rate_input_df")

    expect_not_s3_class(y[1:nrow(y), colnames(y)], "fert_rate_input_df")
})
...
test_that("subset-replacement drops class", {
    ...
    expect_not_s3_class(z, "fert_rate_input_df")
    z <- fert_rate_input_df_time_age
    z$age_start <- z$age_start
    expect_not_s3_class(z, "fert_rate_input_df")
    z <- fert_rate_input_df_time_age
