...
project_ccmpp_loop_over_time <- function(indata) {
  ...
  time_span              <- indata$fert_rate_age_f$time_span[1]
  time_start             <- min(indata$fert_rate_age_f$time_start)
  time_end               <- max(indata$fert_rate_age_f$time_start) + time_span
  ...
  for (time in time_start:(time_end-time_span)) {
    ...
    # mortality
    mort_f            <- indata$survival_ratio_age_sex[which(indata$survival_ratio_age_sex$time_start == time &
                                                            indata$survival_ratio_age_sex$sex == "female"), "value"]
    mort_m            <- indata$survival_ratio_age_sex[which(indata$survival_ratio_age_sex$time_start == time &
                                                            indata$survival_ratio_age_sex$sex == "male"), "value"]
