...
#      toc_float: true
#      includes:
#        in_header: 'css_theme_readable.css' 
# output:
#   word_document:
...
The data frames in `wpp_input_example` have various subsets of the `dimension`s. For example, `wpp_input_example$srb` has only the "time" dimension, the fertility rate component has only the "time" and "age" dimensions. Specific subclasses for these data frames are defined to formalize their characteristics. 
...
|                             | Attributes          |                               |              |
+=============================+=====================+===============================+==============+
| `fert_rate_age_f`           | "non_zero_fert_ages"| c("time", "age")              | "rate"       |
+-----------------------------+---------------------+-------------------------------+--------------+
| `srb`                       |                     | c("time")                     | "ratio"      |
+-----------------------------+---------------------+-------------------------------+--------------+
...
|                             |                     |                               |              |
+-----------------------------+---------------------+-------------------------------+--------------+
| `mig_net_rate_age_sex`      |                     | c("time", "sex", "age")       | "rate"       |
|                             |                     |                               |              |
+-----------------------------+---------------------+-------------------------------+--------------+
| `mig_net_count_age_sex`     |                     | c("time", "sex", "age")       | "count"      |
|                             |                     |                               |              |
+-----------------------------+---------------------+-------------------------------+--------------+
| `mig_net_count_tot_b`       |                     | c("time")                     | "count"      |
+-----------------------------+---------------------+-------------------------------+--------------+
| `mig_parameter`             |                     | c("time")                     | "categorical"|
+-----------------------------+---------------------+-------------------------------+--------------+
| `pop_count_age_sex_base`    |                     | c("time", "sex", "age")       | "count"      |
|                             |                     |                               |              |
+-----------------------------+---------------------+-------------------------------+--------------+
| `life_table_age_sex`        |                     | c("indicator", "time", "sex", | "real"       |
|                             |                     |"age")                         |              |
|                             |                     |                               |              |
+-----------------------------+---------------------+-------------------------------+--------------+
...
The class `ccmpp_input_list` is a list of object with the classes listed in Table \@ref(tab:ccmpp-input-subclass-restrictions). It specifies exactly the inputs needed for a complete run of the CCMPP. It acquires all the attributes of its constituents:
...
Functions `value_type`, `age_span`, `time_span`, `non_zero_fert_ages` are also available to access those attributes for classes that have them. 
