...
Classes for the individual data frame components of the CCMPP lists are all subclasses of `ccmpp_input_df`. Apart from `fert_rate_input_df` they have no additional attributes; none have additional data columns. All have pre-specified dimensions and `value_type`s as described in Table \@ref(tab:ccmpp-input-subclass-restrictions). 

The class `fert_rate_input_df` gets the additional attribute:
