...
4. The data frame must have exactly one value per `age_start` $\times$ `time_start` "Lexis square" for each `indicator` $\times$ `sex` combination. `age_span` and `time_span` are used to expand the `age_start` and `time_start` columns so that each age-time year is checked.
