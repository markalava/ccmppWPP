...
#' Low-level constructor for class \code{mig_net_rate_input_df}.
#'
#' @description
#' Creates an object of class \code{mig_net_rate_input_df}. Minimal
#' checks are done; for interactive use see
#' \code{\link{mig_net_rate_input_df}}.
#'
...
#' Constructor for class \code{mig_net_rate_input_df}
#'
#' \code{mig_net_rate_input_df} is a subclass of
...
mig_net_rate_input_df <-
    function(x) {
        ...
        validate_ccmpp_object(
            new_mig_net_rate_input_df(li$df,
...
#' Coerce to a \code{mig_net_rate_input_df}
#'
...
#' @author Mark Wheldon
#' @name coerce_mig_net_rate_input_df
#' @export
as_mig_net_rate_input_df <- function(x, ...) {
    UseMethod("as_mig_net_rate_input_df")
}

#' @rdname coerce_mig_net_rate_input_df
#' @export
as_mig_net_rate_input_df.default <- function(x, ...) {
    if (is_mig_net_rate_input_df(x)) return(x)
    stop("Cannot coerce 'x' to 'mig_net_rate_input_df'.")
}

#' @rdname coerce_mig_net_rate_input_df
#' @export
as_mig_net_rate_input_df.data.frame <- function(x, ...) {
    mig_net_rate_input_df(as.data.frame(x))
}

#' @rdname coerce_mig_net_rate_input_df
#' @export
as_mig_net_rate_input_df.matrix <- function(x, ...) {
    as_mig_net_rate_input_df(as.data.frame(NextMethod()))
}

#' @rdname coerce_mig_net_rate_input_df
#' @export
as_mig_net_rate_input_df.mig_net_rate_input_df <- function(x, ...) {
    ## copied from  'as.data.frame'
    cl <- oldClass(x)
    i <- match("mig_net_rate_input_df", cl)
    if (i > 1L)
        class(x) <- cl[-(1L:(i - 1L))]
    return(x)
}
