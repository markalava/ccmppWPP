...
ref_pop_years_all <-
    c(1951, 1952, 1953, 1954, 1955, 1956, 1957, 1958, 1959, 1960,
...
ref_pop_years_census <-
    c(1954, 1962, 1968, 1975, 1982, 1990, 1999, 2006, 2011, 2013)
...
test_that("Reference population years can be parsed", {
    x <- parse_pop_count_reference_times_ranges(get_census_years(250))
    expect_equal(x, ref_pop_years_census)
})

test_that("Reference population years can be determined from DDSQL extract", {
    x <- get_pop_count_reference_times(france_wpp_1950_2020_population_data)
    expect_equal(x, ref_pop_years_all)

    x <- exclude_baseline_pop_count_times(france_wpp_1950_2020_population_data,
                                          times = c(1950, 1951))
    expect_equal(x, 1951)
})

test_that("Reference population data can be created", {
    x <- get_pop_count_age_sex_reference(x = france_wpp_1950_2020_population_data)
    ...
    expect_s3_class(x, "demog_change_component_df")
    expect_equal(times(x), ref_pop_years_census)
})
