...
The data frames in `wpp_input_example` have various subsets of the `dimension`s. For example, `wpp_input_example$srb` has only the "time" dimension, the fertility rate component has only the "time" and "age" dimensions. Specific subclasses for these data frames are defined to formalize their characteristics (see below). 
...
3. The data frame has exactly one value per `indicator` $\times$ `age_start` $\times$ `sex` $\times$` time_start` combination. 
4. The rows must be sorted by `indicator`, `time`, `sex`, `age`, varying slowest to fastest in that order. 
5. The only values allowed in the `sex` column are "both", "female", and "male". 
...
2. `age_span` and `time_span` are equal to each other and apply to all ages and times.
3. For every unique `indicator` $\times$ `time_start` $\times$ `sex` combination there is an `age_start` that equals 0; i.e., the age breakdown must start at age 0.
...
:  A list of the values of `age_start` marking the reproductive age groups. Functions can take advantage of this attribute to set fertility to zero in all other age groups regardless of the actual values in the `value` column. 
...
The `print` method for objects of class `fert_rate_age_f` attempts to emphasize the "non_zero_fert_ages" attribute: 
...
To get the names of the dimensions of an object use `demog_change_component_dimensions`^[This name is probably too long.] 

```{r}
demog_change_component_dimensions(demog_change_example_df)
```
...
## Error in validate_ccmpp_object.ccmpp_input_df(new_ccmpp_input_df(li$df,  : 
...
Messages are issued for various reasons during object creation and manipulation. These can be turned off globally with the option "ccmppWPP.suppress_S3_class_messages", e.g., `r options(ccmppWPP.suppress_S3_class_messages = TRUE)`.
