context("Test methods for S3 class 'mortality_rate_age_sex'")

### MAKE OBJECT AVAILABLE TO REMAINDER OF TESTS

mortality_rate_input_df_time_age_sex <-
    subset(wpp_input_example$life_table_age_sex,
           indicator == "lt_nMx", select = -indicator)


test_that("valid member created", {
    expect_s3_class(mortality_rate_age_sex(mortality_rate_input_df_time_age_sex),
...
test_that("successfully extracted from 'life_table...' object", {
    expect_s3_class(mortality_rate_component(
        life_table_input_df_indicator_time_age_sex),
        "mortality_rate_age_sex")
    })
...
test_that("Non-zero age detected", {
    y <- mortality_rate_input_df_time_age_sex
    z <- subset(y, age_start > 0)
    z <- ccmppWPP:::new_mortality_rate_age_sex(ccmppWPP:::sort_demog_change_component_df(z),
                                     age_span = age_span(mortality_rate_age_sex(y)),
                                     time_span = time_span(mortality_rate_age_sex(y)))
...
test_that("Required dimensions enforced", {
    x <- mortality_rate_input_df_time_age_sex
    expect_error(mortality_rate_age_sex(subset(as.data.frame(x),
                                        select =
...
test_that("Invalid 'value's are caught", {
    x <- mortality_rate_input_df_time_age_sex
    x[1, "value"]  <- -1
    expect_error(mortality_rate_age_sex(x),
...
test_that("Equal sex values are detected", {
    ...
    expect_warning(mortality_rate_age_sex(y),
