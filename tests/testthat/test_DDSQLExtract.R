...
test_that("Reference population years can be determined from DDSQL extract", {
    x <- DDextract_get_pop_count_reference_times(france_wpp_1950_2020_population_data)
    expect_equal(x, ref_pop_years_all)

    x <- DDextract_exclude_baseline_pop_count_times(france_wpp_1950_2020_population_data,
                                          times = c(1950, 1951))
    expect_equal(x, 1951)
})
